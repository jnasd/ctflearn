import base64 as rtfd

sik ="1h4ck="

def crypt(sor):
    sro = []
    fusc = "893"
    for i in range(len(sor)):
        sro.append(sor[i]+str(i))
    sro.reverse()
    for i in sro:
        fusc+=i
    return fusc

def decrypt(ros):
    if (len(ros) < 4):
        exit(-1)
    
    sor = ros[3:]       # skip "893"
    first = len(sor) - 2    # point to the last second letter
    i = 1
    fusc = ""
    while (first >= 0):
       fusc += sor[first]
       first -= (len(str(i)) + 1)
       i += 1      # skip str(i) in crypt

    return fusc

def mikeSwift(cre):
    sto = []
    gre = ""
    for i in cre:
        sto.append(i+str(len(i)))
        sto.append("h4ck" + i)
    for i in sto:
        gre+=i
    return gre

def find_all(sub,s):
	index_list = []
	index = s.find(sub)
	while index != -1:
		index_list.append(index)
		index = s.find(sub,index+1)
	if len(index_list) > 0:
		return index_list
	else:
		return -1

def deMikeSwift(cre):
    index_list = find_all("h4ck", cre)
    res = ""
    for index in index_list:
        res+=cre[index + 4]         # skip 'h4ck'
    return res

def obfuscate(bys):
    fusc = rtfd.b64encode(bys)
    fusc += b"534345fdfgfgfdhty6y56yjl"
    fusc = str(fusc)
    fusc = fusc[2:len(fusc)-1]
    refus = []
    for i in fusc:
        refus.append((str(i)))
    fusc="florSFIUEfet4565477"
    for i in refus:
        fusc+=i
    return fusc

def deObfuscate(fusc):
    i = len("florSFIUEfet4565477")
    index = fusc.find("534345fdfgfgfdhty6y56yjl")
    if (index == -1):
        exit(-1)
    basecodes = fusc[i:index]
    return str(rtfd.b64decode(basecodes))

#print(crypt('abcdefghijklmn'))
#print(decrypt(crypt('abcdefghijklmn')))
demike = deMikeSwift(sik)
if (mikeSwift(demike) == sik):
    print("deMikeSwift is right!")
    print(demike)
decry = decrypt(demike)
if (crypt(decry) == demike):
    print("decrypte is right!")
    print(decry)
print("Flag is: " + deObfuscate(decry))
